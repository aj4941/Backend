...
public class TestExitController {
    ...
    public TestResultResponse saveAttemptedProblemResultByGet(HttpSession session) {
        return  (TestResultResponse) session.getAttribute("testResultDto");
    }

    @PostMapping("/submit")
    @ResponseStatus(HttpStatus.OK)
    @Operation(summary = "테스트 종료 시점 코드 저장", description = "테스트 종료 시점에서 종료하기 직전 API를 호출하여 사용자가 풀어둔 코드를 저장해야 합니다.")
    public void saveCode(@RequestBody AttemptCodeRequest attemptCodeDto) {
